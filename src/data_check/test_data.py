...
def test_row_count(data: pd.DataFrame) -> None:
    ...
    Too few rows means the sample is not representative;
    too many means there might be a problem with deduplication.
    """
...
def test_price_range(data: pd.DataFrame, min_price: float, max_price: float) -> None:
    """
    Test that all prices in the cleaned dataset fall within the expected
    [min_price, max_price] range. This guards against the basic_cleaning
    step silently failing to drop outliers.
    """
